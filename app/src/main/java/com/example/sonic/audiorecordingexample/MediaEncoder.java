...
public abstract class MediaEncoder implements Runnable {
    ...
    public boolean frameAvailableSoon() {
    	if (DEBUG) Log.v(TAG, "frameAvailableSoon:::"+mSync);
        synchronized (mSync) {
...
//		mMediaCodec.signalEndOfInputStream();	// API >= 18
    ...
    protected void encode(final ByteBuffer buffer, final int length, final long presentationTimeUs) {
		...
		Log.v(TAG,"[MediaEncoder][AudioThread][encode()]:::"+buffer); // java.nio.DirectByteBuffer[position=0,limit=1024,capacity=1024]
		Log.v(TAG,"[MediaEncoder][AudioThread][encode()]:::"+length); // 1024
		Log.v(TAG,"[MediaEncoder][AudioThread][encode()]:::"+presentationTimeUs); // 192277422059
